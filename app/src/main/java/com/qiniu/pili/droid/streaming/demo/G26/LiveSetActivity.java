...
public class LiveSetActivity extends AppCompatActivity {
    private static final String TAG = "LiveSetActivity";
    ...
    private void updateInputTextView(final String url) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Cache.saveURL(LiveSetActivity.this, url);
            }
        });
    }
    ...
    public void launchStreaming(View v) {
        ...
        Intent intent3 = new Intent(this, ACTIVITY_CLASSES[pos]);
        intent3.putExtra(StreamingBaseActivity.INPUT_TYPE, streamType);
        intent3.putExtra(StreamingBaseActivity.INPUT_TEXT, streamText);
        intent3.putExtras(mEncodingConfigFragment.getIntent());
        boolean bAudioStereo = ((CheckBox) findViewById(R.id.audio_channel_stereo)).isChecked();
        if (bAudioStereo) {
            intent3.putExtra(StreamingBaseActivity.AUDIO_CHANNEL_STEREO, bAudioStereo);
        }
        if (pos == 0) {
            intent3.putExtras(mCameraConfigFragment.getIntent());
        }
    ...
    public void onClickGenPublishURL(View v) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (publishUrl != null) {
                    Cache.saveURL(LiveSetActivity.this, publishUrl);
    ...
    private String genPublishURL() {
        ...
        if (publishUrl == null) {
            Util.showToast(LiveSetActivity.this, "Get publish GENERATE_STREAM_TEXT_V1 failed !!!");
            return null;
        }
